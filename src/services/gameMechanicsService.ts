...
async function analyzeMobDrops(
  ...
  return [
    ...
    {
      key: 'mabuEgg', title: 'Quả Trứng từ mob #70', description: 'Drop riêng trong patch/SA.', baseChancePercent: 5, quantity: 1, group: 'conditional', condition: 'Chỉ khi mob.cG == 70; random(100) < 5.', items: [itemRef(itemAnalysis, 568)], source: source('patch/SA','maybeDropMabuEgg',eggDirect.found,saClass ? 'mob type 70 + w(100) < 5 → item #568' : 'Không có patch/SA trong JAR này',mabuEgg?.descriptor,eggDirect.offset), editableChance: Boolean(saClass && eggDirect.found), editableQuantity: Boolean(saClass && eggDirect.found),
    },
    {
      key: 'activationLevel1', title: 'Trang bị kích hoạt cấp 1', description: 'Patch SA tạo một món trang bị kích hoạt ngẫu nhiên phù hợp hành tinh.', baseChancePercent: 0.1, quantity: 1, group: 'conditional', condition: 'Map 1–3 / 8–10 / 15–17 và mob HP đúng 200 / 500 / 600. Base 10/10.000; nếu có item #1635 thì 12/10.000.', items: [], source: source('patch/SA','maybeDropLevel1Activation',activationVerified,!saClass ? 'Không có patch/SA trong JAR này' : activationVerified ? 'w(10000) < 10; có Cỏ bốn lá #1635 → threshold 12' : 'Không xác minh đủ chuỗi 10000 / 10 / 12 / 1635',activation?.descriptor,activation ? findNumericOffset(activation, saCp, 10000) : undefined), editableChance: Boolean(saClass && activationVerified), editableQuantity: false, notes: ['Tỷ lệ khi có Cỏ bốn lá là 0,12%, cao hơn base 0,10%.'],
    },
    {
    ...
    {
      key: 'campGold', title: 'Vàng mob doanh trại', description: 'Mỗi mob doanh trại tạo một drop vàng cố định trước khi roll Ngọc Rồng.', baseChancePercent: 100, quantity: 100000, group: 'special', condition: 'Map ID 53–62; patch/TM.dropCampMobLoot gọi tm$drop5(item #190, quantity 100000).', items: [itemRef(itemAnalysis,190)], source: source('patch/TM','dropCampMobLoot',campVerified,!tmClass ? 'Không có patch/TM trong JAR này' : 'item #190 “Vàng”, qty=100000, không có RNG bao quanh lệnh drop',campLoot?.descriptor,campLoot ? findNumericOffset(campLoot,tmCp,100000) : undefined), editableChance: Boolean(tmClass && campVerified), editableQuantity: Boolean(tmClass && campVerified),
    },
    {
      key: 'campDragonBalls', title: 'Loot mob doanh trại (map 53–62)', description: 'Ngoài vàng cố định, random một nhánh Ngọc Rồng.', baseChancePercent: 70, quantity: 1, group: 'special', condition: 'customDrop chuyển sang patch/TM.dropCampMobLoot khi map ID 53–62.', items: [18,19,20].map((id)=>itemRef(itemAnalysis,id)), source: source('patch/TM','dropCampMobLoot',campVerified,!tmClass ? 'Không có patch/TM trong JAR này' : 'Vàng 100.000 luôn rơi; RNG 0..99: <10 #18, <40 #19, <70 #20, còn lại không có Ngọc Rồng',campLoot?.descriptor,campLoot ? findNumericOffset(campLoot,tmCp,100000) : undefined), editableChance: false, distribution: [
