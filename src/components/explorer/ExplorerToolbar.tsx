...
import { EntryType } from '../../types/jar';
    ...
    <div id="explorer-toolbar" className="p-4 border-b border-zinc-800 bg-zinc-950/40 space-y-3">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by path or filename (e.g. GameMidlet, a/a/a/, .png)..."
      ...
      <div className="flex flex-wrap items-center gap-1.5 pt-0.5 text-xs font-mono">
        ...
        >
          ...
          <span>All</span>
          <span className="text-[11px] px-1.5 py-0.2 rounded bg-zinc-900/80 text-zinc-400 border border-zinc-800">
            {totalCount.toLocaleString()}
          </span>
        </button>

        <button
          id="filter-btn-classes"
          type="button"
          onClick={() => onFilterChange('classes')}
          className={`px-3 py-1.5 rounded flex items-center gap-1.5 transition-colors ${
            filter === 'classes'
              ? 'bg-blue-950/60 text-blue-200 border border-blue-800 font-semibold'
              : 'text-zinc-400 hover:text-blue-300 hover:bg-zinc-900 border border-transparent'
          }`}
        >
          <FileCode className="w-3.5 h-3.5 text-blue-400" />
          <span>Classes</span>
          <span className="text-[11px] px-1.5 py-0.2 rounded bg-blue-950 text-blue-300 border border-blue-900/80">
            {classCount.toLocaleString()}
          </span>
        </button>

        <button
          id="filter-btn-images"
          type="button"
          onClick={() => onFilterChange('images')}
          className={`px-3 py-1.5 rounded flex items-center gap-1.5 transition-colors ${
            filter === 'images'
              ? 'bg-amber-950/60 text-amber-200 border border-amber-800 font-semibold'
              : 'text-zinc-400 hover:text-amber-300 hover:bg-zinc-900 border border-transparent'
          }`}
        >
          <ImageIcon className="w-3.5 h-3.5 text-amber-400" />
          <span>Images</span>
          <span className="text-[11px] px-1.5 py-0.2 rounded bg-amber-950 text-amber-300 border border-amber-900/80">
            {imageCount.toLocaleString()}
          </span>
        </button>

        <button
          id="filter-btn-resources"
          type="button"
          onClick={() => onFilterChange('resources')}
          className={`px-3 py-1.5 rounded flex items-center gap-1.5 transition-colors ${
            filter === 'resources'
              ? 'bg-purple-950/60 text-purple-200 border border-purple-800 font-semibold'
              : 'text-zinc-400 hover:text-purple-300 hover:bg-zinc-900 border border-transparent'
          }`}
        >
          <Box className="w-3.5 h-3.5 text-purple-400" />
          <span>Resources</span>
          <span className="text-[11px] px-1.5 py-0.2 rounded bg-purple-950 text-purple-300 border border-purple-900/80">
