...
export function PatchedJarExportSection({
  ...
  // Các mã VALIDATED/PATCHED/ORIGINAL là mã trạng thái kỹ thuật, giữ nguyên để không ảnh hưởng logic.
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-zinc-100 text-sm">
              BƯỚC 11 — Tạo, kiểm tra và xuất JAR đã vá
            </span>
          </div>
          <p className="text-[11px] text-zinc-400">
            Tạo một JAR mới độc lập trong RAM, đóng gói lại, mở kiểm tra lần nữa và xác thực bytecode
            cùng dữ liệu trước khi cho phép tải xuống.
          </p>
        </div>
          ...
          >
            {isExporting ? (
              <>
                ...
                <span>Đang tạo và kiểm tra JAR...</span>
              </>
            ) : (
              <>
                <PackageCheck className="w-4 h-4" />
                <span>Tạo JAR đã vá</span>
              </>
            )}
          </button>
        ) : (
          <div className="flex items-center gap-2">
            {onTestPatchedJar && (
              <button
                ...
                title="Mở tab Chạy thử game và chạy trực tiếp JAR đã vá từ RAM"
              >
                <Play className="w-3.5 h-3.5 fill-current" />
                <span>▶ Chạy thử JAR đã vá</span>
              </button>
            )}
            {onTestOriginalJar && (
              <button
                ...
                title="Mở tab Chạy thử game và chạy JAR gốc"
              >
                <span>Chạy JAR gốc</span>
              </button>
            )}
            <button
              type="button"
      ...
      {!canExport && !isExporting && !exportResult && (
        <div className="p-3 bg-amber-950/30 border border-amber-800/60 rounded-lg text-amber-300 space-y-1.5">
          <div className="flex items-center gap-2 font-bold text-amber-400">
            ...
            <span>
              Chưa đủ điều kiện xuất JAR ({unvalidatedGroups.length} nhóm class chưa được xác thực)
            </span>
          </div>
          <p className="text-[11px] text-amber-300/80 leading-relaxed">
            Chỉ class có trạng thái <strong>VALIDATED</strong> mới được đưa vào JAR đầu ra. Hãy tạo và
            kiểm tra bản dựng thử cho các class sau:
          </p>
          <div className="flex flex-wrap gap-1.5 pt-1">
            {unvalidatedGroups.map((g) => (
              ...
              >
                {g.sourceClass} ({g.modifiedCellCount} ô đã sửa)
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="p-3 bg-zinc-900 border border-zinc-750 rounded-lg space-y-2.5">
          <div className="flex items-center justify-between text-zinc-300 text-[11px]">
            ...
            <span className="text-zinc-400 font-mono">
              Bước {exportProgress.currentStep} / {exportProgress.totalSteps}
            </span>
          </div>
      ...
      {exportError && (
        <div className="p-3 bg-red-950/40 border border-red-800/80 rounded-lg text-red-300 space-y-2">
          <div className="flex items-center gap-2 font-bold text-red-400">
            ...
            <span>XÁC THỰC JAR ĐẦU RA THẤT BẠI</span>
          </div>
          ...
          <div className="flex items-center justify-between pt-1">
            <span className="text-[10px] text-red-400/80">
              Chức năng tải xuống đã bị khóa để tránh xuất file JAR lỗi.
            </span>
            <button
              type="button"
              onClick={onBuildPatchedJar}
        ...
        <div className="p-4 bg-emerald-950/20 border border-emerald-800/60 rounded-xl space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-bold text-emerald-300 text-sm">
                  JAR ĐẦU RA ĐÃ ĐƯỢC XÁC THỰC — SẴN SÀNG XUẤT
                </div>
                <div className="text-[11px] text-emerald-400/70">
                  Tất cả 10 bước kiểm định đều đạt. JAR ứng viên có thể tải xuống.
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
            <div className="p-2 bg-zinc-900/80 border border-zinc-800 rounded-lg space-y-0.5">
              <span className="text-zinc-500 text-[10px]">JAR gốc</span>
              <p className="font-bold text-zinc-200 truncate">{exportResult.metrics.originalFileName}</p>
              <p className="text-[10px] text-zinc-400">
                {(exportResult.metrics.originalFileSize / 1024 / 1024).toFixed(2)} MB &bull;{' '}
                {exportResult.metrics.originalTotalEntries} mục
              </p>
            </div>

            <div className="p-2 bg-emerald-950/40 border border-emerald-800/40 rounded-lg space-y-0.5">
              <span className="text-emerald-400 text-[10px]">JAR đầu ra đã vá</span>
              <p className="font-bold text-emerald-300 truncate">{exportResult.metrics.outputFileName}</p>
              <p className="text-[10px] text-emerald-400/80">
                {(exportResult.metrics.outputFileSize / 1024 / 1024).toFixed(2)} MB &bull;{' '}
                {exportResult.metrics.outputTotalEntries} mục
              </p>
            </div>

            <div className="p-2 bg-zinc-900/80 border border-zinc-800 rounded-lg space-y-0.5">
              <span className="text-zinc-500 text-[10px]">Class đã vá</span>
              <p className="font-bold text-blue-300">
                {exportResult.metrics.validatedClassGroupsCount} class ({exportResult.metrics.patchedClassPaths.join(', ')})
              </p>
              <p className="text-[10px] text-zinc-400">
                {exportResult.metrics.actualModifiedCellsCount} ô đã thay đổi
              </p>
            </div>

            <div className="p-2 bg-zinc-900/80 border border-zinc-800 rounded-lg space-y-0.5">
              <span className="text-zinc-500 text-[10px]">Thay đổi ngoài dự kiến</span>
              <p className="font-bold text-emerald-400">0 thay đổi ngoài dự kiến</p>
              <p className="text-[10px] text-zinc-400">JAR gốc: KHÔNG BỊ THAY ĐỔI</p>
            </div>
          </div>

          <div className="space-y-1.5">
            <span className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider">
              Danh sách kiểm tra sau khi đóng gói và mở lại JAR
            </span>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-[11px]">
              {exportResult.checkSteps.map((step) => (
                <div
                  key={step.name}
          ...
          {exportResult.metrics.signatureDetected && (
            <div className="p-2.5 bg-amber-950/30 border border-amber-800/60 rounded-lg flex items-start gap-2 text-[11px] text-amber-300">
              ...
              <div>
                <span className="font-bold">JAR này có dấu hiệu chứa metadata chữ ký.</span>
                <p className="text-[10px] text-amber-300/80">
                  Việc sửa nội dung class có thể làm chữ ký gốc mất hiệu lực:{' '}
                  {exportResult.metrics.signatureFiles.join(', ')}
                </p>
              </div>
            </div>
          )}
          ...
          {exportResult.semanticDiffs.length > 0 && (
            <div className="space-y-1.5">
              <span className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider">
                Thay đổi dữ liệu đã xác nhận trong bytecode đầu ra ({exportResult.semanticDiffs.length})
              </span>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400 text-[10px]">
                        Dòng {diff.rowIndex} &bull; Cột {diff.columnIndex} ({diff.fieldName})
                      </span>
                    </div>
