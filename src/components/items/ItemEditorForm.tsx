            ...
            {showPicker && (
              <div className="relative mt-3 rounded-xl border border-violet-200 bg-white p-2 shadow-sm">
                <div className="relative">
                  ...
                  <input autoFocus value={optionQuery} onChange={(e) => setOptionQuery(e.target.value)} placeholder="Tìm: HP, KI, sức đánh, giáp, chí mạng, hút, phản, xuyên, né... hoặc Option ID" className="w-full pl-8 pr-3 py-2.5 sm:py-2 rounded-lg border border-zinc-300 text-[16px] sm:text-xs focus:outline-none focus:border-violet-400" />
                </div>
                  ...
                  <div key={override.optionId} className={`rounded-xl border p-3 ${override.enabled ? 'border-violet-200 bg-violet-50/40' : 'border-zinc-200 bg-zinc-50 opacity-70'}`}>
                    <div className="grid grid-cols-1 lg:grid-cols-[minmax(220px,1fr)_180px_auto] gap-2 items-end">
                      ...
                      <label>
                        ...
                        <input type="number" min={JAVA_INT_MIN} max={JAVA_INT_MAX} value={override.param} onChange={(e) => updateOption(override.optionId, { param: Math.max(JAVA_INT_MIN, Math.min(JAVA_INT_MAX, Math.round(Number(e.target.value) || 0))) })} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2.5 sm:py-2 text-[16px] sm:text-xs font-mono focus:outline-none focus:border-violet-400" />
                      </label>
